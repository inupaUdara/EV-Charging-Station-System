  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        toast.error("Error loading station details. Please try again.");
      }
    })();
  }, [id, isEditMode]);
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.name.trim().length < 3) {
        toast.error("Station name must be at least 3 characters long.");
        setLoading(false);
        return;
      }
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
        toast.error("Please select a valid location (latitude/longitude).");
        setLoading(false);
        return;
      }
      ...
      if (!formData.type || !["AC", "DC"].includes(formData.type)) {
        toast.error("Please select a valid charging type (AC or DC).");
        setLoading(false);
        return;
      }
      
      if (formData.schedules.length === 0) {
        toast.error("Please add at least one operating schedule.");
        setLoading(false);
        return;
      }
      ...
      const errorMessage = err?.response?.data?.message || "Failed to save station. Please try again.";
