...
export default function StationList({ onCreate, onView, onEdit }) {
      ...
      render: (r) => (
        <div className="flex gap-2">
          ...
          {r.isActive ? (
            ...
            <Button
              ...
              onClick={async (e) => {
                ...
                if (res.success) {
                  toast.success(`Station "${r.name}" activated successfully!`);
                } else {
                  toast.error(res.error || "Failed to activate station.");
                }
              }}
            >
              Activate
          ...
          <Button
            ...
            onClick={async (e) => {
              e.stopPropagation();
              if (!confirm(`Delete station "${r.name}" permanently? This action cannot be undone.`)) return;
              const res = await handleDeleteStation(r.id || r._id);
              if (res.success) {
                toast.success(`Station "${r.name}" deleted successfully.`);
              } else {
  ...
  const handleConfirm = async () => {
    ...
    const station = stations.find(s => (s.id || s._id) === pendingDeactivateId);
    const stationName = station?.name || "Station";
    
    const res = await handleDeactivateStation(pendingDeactivateId);
    if (res.success) {
      toast.success(`${stationName} deactivated successfully.`);
    } else {
      toast.error(res.error || "Failed to deactivate station.");
    }
